...
public class Cons extends List {
	...
	public int get(int i){
		assert i < size() : "Non esiste un elemento in posizione:" + i + ":(";
		Cons tmp = this;
		if(i == 0){
			return tmp.elem;
		}else{
			return tmp.next.get(i-1);
		}
	}
	...
	public List filter_le(int x){
		return elem <= x ? new Cons(elem, next.filter_le(x)) : next.filter_le(x) ;
	}
	
	@Override
	public List filter_gt(int x){
		return elem > x ? new Cons(elem, next.filter_gt(x)) : next.filter_gt(x) ;
	}
	...
	public List intersect(List l){
		...
		if(elem == l.get(0)){
			...
			return next.intersect(((Cons) l).next);
		}
	}
}
