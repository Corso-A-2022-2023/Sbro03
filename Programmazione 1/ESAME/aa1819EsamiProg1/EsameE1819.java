...
public class EsameE1819 {
	 ...
	 */
	 ...
	 static boolean e1(int[][] m, int[] a){
		 boolean ogni = true;
		 if(m == null || a == null)
			 ogni = false;
		 else{			 
			 boolean flag = true;
			 for(int e = 0; e < m.length && flag; e++){
				 if(m[e] != null)
					flag = m[0].length == m[e].length;
				else flag = false;
			 }
			 if(flag){	
				 for(int i = 0; i < m.length && ogni; i++){
					 boolean trovato = false; 
					 if(m[i] != null)
						for(int j = 0; j < a.length && !trovato; j++){
							if(i < m[i].length)
							   trovato = a[j] > m[i][i];
							else trovato = true;
						}
	...
	public static boolean e2(int[] a, int f, int d){
		if(a != null && f > 0 && f < a.length-1)
			return e2R(a,0,f,d);
		else return true;
	}
	public static boolean e2R(int[] a, int pos, int f, int d){
		boolean flag = false;
		if(pos == f){
			flag = a[pos] % d == 0;
		}else{
			flag = e2R(a, pos+1, f, d);
		}
		return flag;
	}
